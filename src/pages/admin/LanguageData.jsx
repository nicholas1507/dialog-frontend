...
const LanguageData = () => {
  ...
  const loadLanguages = async (page = 1, keyword = "") => {
    setLoading(true);
    setError(null);
    try {
      const res = await getLanguages({ page, limit: pagination.limit, search: keyword });
      setLanguages(res.data || (Array.isArray(res) ? res : []));
      if (res.pagination) {
  ...
  useEffect(() => {
    loadLanguages(currentPage, search);
  }, [currentPage, search]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await loadLanguages(currentPage, search);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedData?.id) {
        await updateLanguage(selectedData.id, formData);
      } else {
  ...
  return (
    <div className="container-fluid px-4 py-4">
      <CCard className="border-0 shadow-sm rounded-4">
        <CCardBody className="p-4">
          <div className="d-flex justify-content-between align-items-center mb-4">
            <div>
              <h4 className="fw-bold mb-1">Languages</h4>
              <p className="text-muted small mb-0">Manage language options for translation services</p>
            </div>
      ...
      <CModal visible={visible} onClose={() => setVisible(false)} alignment="center">
        <CModalHeader>
          <CModalTitle>{selectedData ? "Edit Language" : "Add Language"}</CModalTitle>
        </CModalHeader>
        <CForm onSubmit={handleSubmit}>
          <CModalBody className="d-flex flex-column gap-3">
            ...
            <div>
              <label className="form-label fw-semibold small text-uppercase">Language Name</label>
              <CFormInput
            ...
            <div>
              <label className="form-label fw-semibold small text-uppercase">Language Code</label>
              <CFormInput
                placeholder="e.g. en"
                value={formData.code}
